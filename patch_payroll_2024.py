...
def calculate_tax_wedge(base_cash_wage, status='single', num_children=0, 
    ...
    # --- Payroll Taxes (2024 Exact Limits) ---
    # Social Security Tax (Employer and Employee)
    ss_wage_base = 168600
    ss_rate_employer = 0.062
    ss_rate_employee = 0.062
    
    ss_tax_employer = min(base_cash_wage, ss_wage_base) * ss_rate_employer
    ss_tax_employee = min(base_cash_wage, ss_wage_base) * ss_rate_employee
    
    # Medicare Tax (Employer and Employee)
    medicare_rate_employer = 0.0145
    medicare_rate_employee = 0.0145
    
    medicare_tax_employer = base_cash_wage * medicare_rate_employer
    medicare_tax_employee = base_cash_wage * medicare_rate_employee
    
    # Additional Medicare Tax (Employee Only)
    add_medicare_rate = 0.009
    add_medicare_threshold = 200000 if status == 'single' else 250000
    add_medicare_tax = max(0, base_cash_wage - add_medicare_threshold) * add_medicare_rate
    
    total_employee_payroll_tax = ss_tax_employee + medicare_tax_employee + add_medicare_tax
    total_employer_payroll_tax = ss_tax_employer + medicare_tax_employer

    # 1. Gross Employer Cost
    health_cost = base_cash_wage * health_ratio
    pension_cost = base_cash_wage * pension_ratio
    employer_payroll = total_employer_payroll_tax
    ...
    employee_payroll = total_employee_payroll_tax
...
def plot_reform_impact(rate=0.25, adult_credit=4800, child_credit=4800):
    ...
    plot_filename = f'Reform_Impact_{rat
