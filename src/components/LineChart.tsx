...
export function LineChart({ points, title, aLabel, bLabel, percentAxis = false, xScale = 'linear', yDomain }: {
  ...
  return (
    <figure className="line-chart">
      ...
      <div className="chart-legend"><span className="legend-a">{aLabel}</span><span className="legend-b">{bLabel}</span><small>Move across the chart to trace values</small></div>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={title} onPointerMove={trace} onPointerLeave={() => setHovered(null)}>
        ...
        {active && <g className="chart-trace">
          ...
          <g transform={`translate(${tooltipX},${pad.top + 8})`}>
            <rect width={tooltipWidth} height="82" rx="9" />
