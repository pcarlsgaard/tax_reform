...
export function Household({ settings }: { settings: ReformSettings }) {
  ...
  const [chartMax, setChartMax] = useState(500000);
  ...
  const chartPoints = useMemo(() => Array.from({ length: 121 }, (_, index) => {
    const cashWage = index * chartMax / 120;
  ...
  }), [input.filingStatus, input.children, input.secondaryCashWage, chartMax, settings]);
  ...
  return <main className="page-shell household-page">
    ...
    <section className="chart-toolbar"><div><h2>Trace household results across earnings</h2><p>Charts vary the primary wage while holding the selected spouse wage and household type fixed.</p></div><label><span>Maximum primary wage</span><select value={chartMax} onChange={(event) => setChartMax(Number(event.target.value))}><option value={250000}>$250,000</option><option value={500000}>$500,000</option><option value={1000000}>$1,000,000</option></select></label></section>
    <section className="chart-stack"><LineChart title="Disposable resources across primary cash wages" points={chartPoints.map((point) => ({ x: point.x, ...point.disposable }))} aLabel="Current law" bLabel="Reform" /><LineChart title="Average effective federal tax rate" points={chartPoints.map((point) => ({ x: point.x, ...point.average }))} aLabel="Current law" bLabel="Reform" percentAxis /><LineChart title="Local marginal federal tax rate · $1,000 window" points={chartPoints.map((point) => ({ x: point.x, ...point.marginal }))} aLabel="Current law" bLabel="Reform" percentAxis /></section>
    ...
    <section className="table-card wedge-table">
      ...
      <div className="responsive-table"><table><thead><tr><th>Family / earnings pattern</th><th>Primary wage</th><th>Spouse wage</th><th>Employer compensation</th><th>Current wedge</th><th>Reform wedge</th><th>Change</th></tr></thead><tbody>{wedgeRows.map((row) => <tr key={row.id}><td>{row.label}</td><td>{dollars(row.primaryWage)}</td><td>{row.secondaryWage ? dollars(row.secondaryWage) : '—'}</td><td>{dollars(row.employerCompensation)}</td><td>{percent(row.currentWedge)}</td><td>{percent(row.reformWedge)}</td><td className={row.changePercentagePoints <= 0 ? 'good-text' : 'bad-text'}>{row.changePercentagePoints.toFixed(1)} pp</td></tr>)}</tbody></table></div>
