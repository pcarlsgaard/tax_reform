...
export function Household({ settings }: { settings: ReformSettings }) {
  const [input, setInput] = useState<HouseholdInput>({ filingStatus: 'married', children: 2, childrenUnder6: 1, cashWage: 100000, secondaryCashWage: 0 });
  ...
  }), [input.filingStatus, input.children, input.childrenUnder6, input.secondaryCashWage, chartMax, chartScale, settings]);
  ...
  return <main className="page-shell household-page">
    ...
    <section className={`household-inputs ${input.filingStatus === 'married' ? 'four-up' : ''}`}>
      ...
      <label><span>Children</span><select value={input.children} onChange={(event) => { const children = Number(event.target.value); update({ children, childrenUnder6: Math.min(input.childrenUnder6 ?? 0, children) }); }}>{[0,1,2,3,4].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
      <label><span>Children under 6</span><select value={Math.min(input.childrenUnder6 ?? 0, input.children)} onChange={(event) => update({ childrenUnder6: Number(event.target.value) })}>{Array.from({ length: input.children + 1 }, (_, value) => <option key={value} value={value}>{value}</option>)}</select></label>
    ...
    <div className="metric-grid household-metrics">
      ...
      <MetricCard label="Current net federal position" value={taxPosition(result.current.totalFederalTax)} note={`Average rate ${percent(result.currentAverageRate)}`}><CurrentAudit result={result} /></MetricCard>
      <MetricCard label="Reform net federal position" value={taxPosition(result.reformTaxAfterCredits)} note={`Average rate ${percent(result.reformAverageRate)}`}><ReformAudit result={result} settings={settings} /></MetricCard>
    ...
    <section className="table-card compact"><h2>Calculation detail</h2><div className="responsive-table"><table><thead><tr><th>Measure</th><th>Current law</th><th>Reform</th></tr></thead><tbody><tr><td>Gross resources before federal tax</td><td>{dollars(result.employerCompensation)}</td><td>{dollars(result.reformGrossResources)}</td></tr><tr><td>Wage base after named exemptions</td><td>—</td><td>{dollars(result.taxableReformWageBase)}</td></tr><tr><td>Subtract: pre-credit federal tax liability</td><td>{dollars(result.currentPreCreditTaxLiability)}</td><td>{dollars(result.reformPreCreditTaxLiability)}</td></tr><tr><td>Add: adult credit</td><td>—</td><td>{dollars(result.adultCredit)} of {dollars(result.adultCreditMaximum)} maximum</td></tr><tr><td>Add: child credit</td><td>CTC/ACTC below</td><td>{dollars(result.childCredit)} · {result.input.childrenUnder6 ?? 0} under 6</td></tr><tr><td>Add: EITC</td><td>{dollars(result.current.eitc)}</td><td>{settings.replacedTaxes.individualIncome ? 'replaced' : dollars(result.current.eitc)}</td></tr><tr><td>Add: CTC + ACTC</td><td>{dollars(result.current.nonrefundableCtc + result.current.refundableCtc)}</td><td>{settings.replacedTaxes.individualIncome ? 'replaced' : dollars(result.current.nonrefundableCtc + result.current.refundableCtc)}</td></tr><tr className="subtotal"><td>Disposable resources</td><td>{dollars(result.currentDisposableResources)}</td><td>{dollars(result.reformDisposableResources)}</td></tr><tr><td>Average effective rate</td><td>{percent(result.currentAverageRate)}</td><td>{percent(result.reformAverageRate)}</td></tr><tr><td>Local marginal rate</td><td>{percent(result.currentMarginalRate)}</td><td>{percent(result.reformMarginalRate)}</td></tr></tbody></table><p className="table-note">Named cash-wage and employer-social-insurance exemptions affect the household tax base but not gross compensation used for adult- or child-credit phase-in. Employer pension and insurance benefits are not present in this wage-only household example. Local marginal rates use a centered $1,000 earnings window.</p></div></section>
    ...
    <p className="method-note">Static wage-only illustration. It assumes all adults are working-age, all selected children qualify for the CTC, the entered under-6 count receives the supplement, no itemized deductions or nonwage income, and dollar-for-dollar conversion of repealed employer payroll taxes into compensation. Employer pension and health benefits are omitted from this chart; the national revenue base includes them. See the <a href="https://github.com/pcarlsgaard/tax_reform/blob/agent/dbcft-general-equilibrium-v1/analysis/wage_tax_credit_diagnostic_2025.md" target="_blank" rel="noreferrer">worker-level comparison of both bases</a>. It is not a microsimulation.</p>
  </main>;
}
...
function CurrentAudit({ result }: { result: ReturnType<typeof calculateHousehold> }) {
  ...
  return <Audit><Formula>{dollars(result.employerCompensation)} gross resources<br />− {dollars(result.currentPreCreditTaxLiability)} pre-credit federal liability<br />+ {dollars(result.currentTaxCredits)} current tax credits<br />= {dollars(result.currentDisposableResources)} disposable resources<br /><br />Net federal position: {taxPosition(c.totalFederalTax)}.</Formula></Audit>;
}

function ReformAudit({ result, settings }: { result: ReturnType<typeof calculateHousehold>; settings: ReformSettings }) {
  return <Audit><Formula>{dollars(result.reformGrossResources)} gross resources<br />− {dollars(result.reformPreCreditTaxLiability)} pre-credit federal liability ({settings.wageTaxMode} wage tax plus retained taxes)<br />+ {dollars(result.reformTotalCredits)} household credits<br />= {dollars(result.reformDisposableResources)} disposable resources<br /><br />Net federal position: {taxPosition(result.reformTaxAfterCredits)}.</Formula></Audit>;
}

function taxPosition(value: number): string {
  if (value < 0) return `${dollars(-value)} refund`;
  return `${dollars(value)} tax`;
}
