...
export function National({ settings, result }: { settings: ReformSettings; result: MacroResult }) {
  ...
  return <main className="page-shell">
    ...
    <section className="table-card compact">
      ...
      {result.totalFederalSavings > 0 && <Audit title="Audit fiscal savings"><Formula>{moneyB(result.targetRevenue)} selected current-law receipts<br />− {moneyB(result.refundableTaxCreditOutlaySavings)} refundable EITC/CTC outlays automatically removed with individual income taxation<br />− {moneyB(result.federalTransferSavings)} selected external federal program savings<br />= {moneyB(result.adjustedTargetRevenue)} current-law replacement baseline.<br /><br />Only refundable excess payments are added here. Credit amounts that offset positive tax liability already reduce the individual-income-tax receipts comparison.<br /><br />Replacement-neutral rate: {percent(result.revenueNeutralRate, 2)} before savings → {percent(result.adjustedRevenueNeutralRate, 2)} after savings, a {(result.revenueNeutralRateReduction * 100).toFixed(2)} percentage-point reduction.</Formula></Audit>}
    </section>
