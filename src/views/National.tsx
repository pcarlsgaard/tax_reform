  ...
  return <main className="page-shell">
    ...
    <section className="table-card compact">
      ...
      <div className="responsive-table"><table><thead><tr><th>Revenue item</th><th>Value</th><th>% GDP</th></tr></thead><tbody>
        ...
        <tr><td>Selected current-law receipts replaced</td><td>{moneyB(result.targetRevenue)}</td><td>{percent(result.targetRevenuePercentGdp)}</td></tr>
        ...
        <tr><td>Selected program and estimated ACA-credit savings</td><td>−{moneyB(result.federalTransferSavings)}</td><td>−{percent(result.federalTransferSavings / result.gdp)}</td></tr>
        ...
        <tr className="subtotal"><td>Current-law replacement baseline</td><td>{moneyB(result.adjustedTargetRevenue)}</td><td>{percent(result.adjustedTargetRevenuePercentGdp)}</td></tr>
        <tr className="total"><td>Static deficit {result.deficitReduction >= 0 ? 'reduction' : 'increase'}</td><td>{moneyB(result.deficitReduction)}</td><td>{percent(result.deficitReductionPercentGdp)}</td></tr>
      </tbody></table></div>
      ...
      <Audit title="Audit the static deficit change"><Formula>{moneyB(result.netRevenue)} reform net revenue<br />− {moneyB(result.targetRevenue)} selected current-law receipts replaced<br />+ {moneyB(result.totalFederalSavings)} eliminated federal outlays<br />= {moneyB(result.deficitReduction)} static deficit {result.deficitReduction >= 0 ? 'reduction' : 'increase'}.<br /><br />All other federal receipts and outlays are held fixed, so they cancel from this relative comparison.</Formula></Audit>
      {result.totalFederalSavings > 0 && <Audit title="Audit fiscal savings"><Formula>{moneyB(result.targetRevenue)} selected current-law receipts<br />− {moneyB(result.refundableTaxCreditOutlaySavings)} refundable EITC/CTC outlays automatically removed with individual income taxation<br />− {moneyB(result.federalTransferSavings)} selected program and estimated displaced ACA premium-credit savings<br />= {moneyB(result.adjustedTargetRevenue)} current-law replacement baseline.<br /><br />The ACA premium-credit amount is an estimate from linked survey amounts, not reconciled FY2025 administrative outlays. Credit amounts that offset positive income-tax liability already reduce receipts and should not be added again.<br /><br />Replacement-neutral rate: {percent(result.revenueNeutralRate, 2)} before savings → {percent(result.adjustedRevenueNeutralRate, 2)} after savings, a {(result.revenueNeutralRateReduction * 100).toFixed(2)} percentage-point reduction.</Formula></Audit>}
    </section>
