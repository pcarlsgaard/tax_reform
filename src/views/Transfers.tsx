  ...
  return <main className="page-shell transfer-page">
    ...
    <section className="table-card compact decomposition-table">
      ...
      <div className="responsive-table"><table><thead><tr><th>Measure</th><th>Current law</th><th>Reform + transfers</th><th>Reform + selected replacements</th></tr></thead><tbody>
        <tr><td>Gross household resources before federal tax</td><td>{dollars(analysis.tax.employerCompensation)}</td><td>{dollars(analysis.tax.reformGrossResources)}</td><td>{dollars(analysis.tax.reformGrossResources)}</td></tr>
        <tr><td>Employer FICA pass-through (memorandum)</td><td>—</td><td>{settings.replacedTaxes.payroll ? `${dollars(analysis.tax.employerFicaPassThrough)} at ${percent(analysis.tax.employerFicaPassThroughRate)}` : 'not applicable—payroll retained'}</td><td>{settings.replacedTaxes.payroll ? `${dollars(analysis.tax.employerFicaPassThrough)} at ${percent(analysis.tax.employerFicaPassThroughRate)}` : 'not applicable—payroll retained'}</td></tr>
      ...
      <Audit><Formula>Current gross resources include cash wages plus employer FICA, with employer FICA then included in current federal tax. When payroll taxes are replaced, reform gross resources include cash wages plus the selected pass-through share of that employer FICA. The non-passed-through share is not assigned to the household in this illustrative incidence sensitivity. EITC and CTC/ACTC are memorandum rows only: current disposable resources already include them through net individual income tax. Reform adult/child credits likewise already enter reform tax after credits. Adding either row again would double-count them.</Formula></Audit>
    </section>
