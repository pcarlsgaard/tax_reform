...
export function Transfers({
  ...
  return <main className="page-shell transfer-page">
    ...
    <section className="table-card compact scenario-table">
      ...
      <Audit title="Audit the resource identity"><Formula>Current law = {dollars(analysis.currentGrossResources)} cash wages − {dollars(analysis.currentPreCreditTaxLiability)} household pre-credit federal tax + {dollars(analysis.currentTaxCredits)} current tax credits + {dollars(analysis.totalCurrentExternalTransfers)} external transfers = {dollars(analysis.currentLaw.annual)}. Employer FICA is neither added to current household cash nor subtracted as a household tax.<br /><br />{settings.replacedTaxes.payroll ? <>Reform-side employer FICA pass-through = {percent(analysis.tax.employerFicaPassThroughRate)} × {dollars(analysis.tax.current.employerPayrollTax)} current employer FICA = {dollars(analysis.tax.employerFicaPassThrough)}.</> : <>Payroll taxes are retained, so no employer FICA is passed through to reform household resources.</>}<br /><br />Reform with transfers = {dollars(analysis.reformGrossResources)} cash resources after pass-through − {dollars(analysis.reformPreCreditTaxLiability)} household pre-credit federal tax + {dollars(analysis.reformTaxCredits)} tax credits + {dollars(analysis.totalCurrentExternalTransfers)} external transfers = {dollars(analysis.reformRetained.annual)}.<br /><br />Reform after replacement = {dollars(analysis.reformGrossResources)} cash resources − {dollars(analysis.reformPreCreditTaxLiability)} household pre-credit federal tax + {dollars(analysis.reformTaxCredits)} tax credits + {dollars(analysis.totalCurrentExternalTransfers)} external transfers − {dollars(analysis.eliminatedHouseholdBenefits)} selected benefit replacements = {dollars(analysis.reformAfterReplacement.annual)}. No negative tax is subtracted, and no amount is divided by 1 + the tax rate.</Formula></Audit>
    </section>
    ...
    <section className="table-card compact decomposition-table">
      ...
      <div className="responsive-table"><table><thead><tr><th>Measure</th><th>Current law</th><th>Reform + transfers</th><th>Reform + selected replacements</th></tr></thead><tbody>
        ...
        <tr><td>Add: employer FICA pass-through</td><td>—</td><td>{settings.replacedTaxes.payroll ? `${dollars(analysis.tax.employerFicaPassThrough)} at ${percent(analysis.tax.employerFicaPassThroughRate)}` : '— (payroll retained)'}</td><td>{settings.replacedTaxes.payroll ? `${dollars(analysis.tax.employerFicaPassThrough)} at ${percent(analysis.tax.employerFicaPassThroughRate)}` : '— (payroll retained)'}</td></tr>
        <tr className="subtotal"><td>Gross household cash resources before federal tax</td><td>{dollars(analysis.currentGrossResources)}</td><td>{dollars(analysis.reformGrossResources)}</td><td>{dollars(analysis.reformGrossResources)}</td></tr>
        <tr><td>Subtract: household pre-credit federal tax liability</td><td>{dollars(analysis.currentPreCreditTaxLiability)}</td><td>{dollars(analysis.reformPreCreditTaxLiability)}</td><td>{dollars(analysis.reformPreCreditTaxLiability)}</td></tr>
      ...
      <Audit><Formula>Every row above is part of the displayed cash-resource identity. Employer FICA is not current household cash and is not a current household tax in this table. Only the selected share of repealed employer FICA is added on the reform side. Current-law credits are added after household-side pre-credit liability; reform credits are added after reform household-side liability. External transfers are then added, and only the selected replacement value is subtracted in the final column.</Formula></Audit>
    </section>
