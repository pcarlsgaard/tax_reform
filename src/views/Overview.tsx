    ...
    <div className="view-grid overview-view">
      ...
      <main className="results-panel">
        ...
        <div className="metric-grid">
          ...
          <MetricCard label="Replacement-neutral rate" value={percent(result.adjustedRevenueNeutralRate, 2)} note={hasFiscalSavings ? `Before fiscal savings ${percent(result.revenueNeutralRate, 2)}` : 'Solved algebraically'} tone="accent"><Audit><Formula>{hasFiscalSavings && <>{moneyB(result.targetRevenue)} selected current-law receipts<br />− {moneyB(result.refundableTaxCreditOutlaySavings)} automatic refundable EITC/CTC outlay savings<br />− {moneyB(result.federalTransferSavings)} selected external-program savings<br />= {moneyB(result.adjustedTargetRevenue)} current-law replacement baseline.<br /><br /></>}({moneyB(result.adjustedTargetRevenue)} + {moneyB(result.adultCreditCost + result.childCreditCost + result.insuranceCreditCost)}) ÷ {moneyB(result.rateAdjustedBase)} rate-adjusted base = {percent(result.adjustedRevenueNeutralRate, 2)}</Formula></Audit></MetricCard>
        </div>
