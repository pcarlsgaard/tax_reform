    ...
    <div className="view-grid overview-view">
      <aside className="control-panel">
        ...
        {settings.wageTaxMode === 'progressive' && <div className="control-group">
          ...
          <label className="control-note"><input type="checkbox" checked={settings.progressiveIntermediateStartPerAdult !== null} onChange={(event) => update(event.target.checked
            ? { progressiveIntermediateStartPerAdult: 75000, progressiveTopBracketPerAdult: 200000, progressiveIntermediateRate: 0.30 }
            : { progressiveIntermediateStartPerAdult: null })} /> Add a 30% intermediate wage bracket</label>
          {settings.progressiveIntermediateStartPerAdult !== null && <>
            <RangeField label="Intermediate starts / adult" value={settings.progressiveIntermediateStartPerAdult} min={0} max={300000} step={5000} display={`$${settings.progressiveIntermediateStartPerAdult.toLocaleString()}`} onChange={(progressiveIntermediateStartPerAdult) => update({ progressiveIntermediateStartPerAdult })} hint="Applied to taxable household compensation; the 35% bracket starts at the top-rate threshold above." />
            <RangeField label="Intermediate wage rate" value={settings.progressiveIntermediateRate} min={settings.progressiveMiddleRate} max={settings.rate} step={0.005} display={percent(settings.progressiveIntermediateRate)} onChange={(progressiveIntermediateRate) => update({ progressiveIntermediateRate })} />
          </>}
        ...
        {settings.adultCreditMode === 'earned' && <div className="control-group">
          ...
          <SelectField label="Cash-out and adult credit" value={settings.adultCreditEarningsBase ?? 'compensation'} options={[{ value: 'cash', label: 'Cash wages + repealed employer payroll contributions' }, { value: 'compensation', label: 'All employer compensation paid as cash' }]} onChange={(adultCreditEarningsBase) => update({ adultCreditEarningsBase: adultCreditEarningsBase as 'cash' | 'compensation' })} hint="Both options tax all compensation alike. Cash-out changes only how quickly the $2,000 earned credit phases in. Individual household examples do not impute pension benefits." />
        ...
        <RangeField label="Child credit" value={settings.childCredit} min={0} max={24000} step={100} display={`$${settings.childCredit.toLocaleString()}`} onChange={(childCredit) => update({ childCredit })} hint="Base amount for every qualifying child under 18." />
      ...
      <main className="results-panel">
        ...
        <div className="metric-grid">
          ...
          <MetricCard label="Net federal revenue" value={moneyB(result.netRevenue)} note={`${percent(result.netRevenuePercentGdp)} of GDP · ${moneyB(Math.abs(result.deficitReduction))} static deficit ${result.deficitReduction >= 0 ? 'reduction' : 'increase'}`} tone={result.deficitReduction >= 0 ? 'good' : 'bad'}><RevenueAudit result={result} /></MetricCard>
          <MetricCard label={`Static deficit ${result.deficitReduction >= 0 ? 'reduction' : 'increase'}`} value={moneyB(result.deficitReduction)} note={`${percent(result.deficitReductionPercentGdp)} of GDP${hasFiscalSavings ? ` · includes ${moneyB(result.totalFederalSavings)} federal savings` : ''}`} tone={result.deficitReduction >= 0 ? 'good' : 'bad'}><Audit><Formula>{moneyB(result.netRevenue)} reform net revenue<br />− {moneyB(result.targetRevenue)} selected current-law receipts replaced<br />+ {moneyB(result.totalFederalSavings)} eliminated federal outlays<br />= {moneyB(result.deficitReduction)} static deficit {result.deficitReduction >= 0 ? 'reduction' : 'increase'}</Formula></Audit></MetricCard>
          <MetricCard label="Replacement-neutral rate" value={percent(result.adjustedRevenueNeutralRate, 2)} note={hasFiscalSavings ? `Before fiscal savings ${percent(result.revenueNeutralRate, 2)}` : 'Solved algebraically'} tone="accent"><Audit><Formula>{hasFiscalSavings && <>{moneyB(result.targetRevenue)} selected current-law receipts<br />− {moneyB(result.refundableTaxCreditOutlaySavings)} automatic refundable EITC/CTC outlay savings<br />− {moneyB(result.federalTransferSavings)} selected programs and estimated ACA premium-credit savings<br />= {moneyB(result.adjustedTargetRevenue)} current-law replacement baseline.<br /><br /></>}({moneyB(result.adjustedTargetRevenue)} + {moneyB(result.adultCreditCost + result.childCreditCost + result.insuranceCreditCost)}) ÷ {moneyB(result.rateAdjustedBase)} rate-adjusted base = {percent(result.adjustedRevenueNeutralRate, 2)}</Formula></Audit></MetricCard>
        </div>
...
function AdultCreditAuditPanel({ result, settings }: { result: MacroResult; settings: ReformSettings }) {
  ...
  return <section className="table-card compact adult-credit-audit">
    ...
    <div className="two-column credit-audit-notes">
      <div className="content-card"><h3>Earned-credit base versus taxable pay</h3><p><strong>{moneyB(controls.cashWagesAndSalaries)}</strong> BEA-raked cash wages + <strong>{moneyB(controls.employerGovernmentSocialInsurance)}</strong> employer government social insurance + <strong>{moneyB(controls.employerPensionAndInsurance)}</strong> employer pension and insurance supplements = <strong>{moneyB(controls.totalCompensation)}</strong> gross employee compensation.</p><p>Reform taxes all these compensation components as wages. The earned adult credit {settings.adultCreditEarningsBase === 'cash' ? 'phases in on original wages plus imputed passed-through employer social insurance; benefits paid in kind are excluded' : 'phases in on gross compensation, as in the full cash-out sensitivity'}. Self-employment income is not included. Tax-base exemptions and noncompliance do not change credit-eligibility earnings.</p></div>
    ...
    <Audit title="Audit the live calculation identity"><Formula>For each CPS tax unit:<br />maximum = eligible adults × {dollars(settings.adultCredit)}<br />phase-in = min(maximum, {settings.adultCreditEarningsBase === 'cash' ? 'cash wages' : 'gross employee compensation'} × {percent(settings.adultCreditPhaseInRate)})<br />phaseout = max(0, {settings.adultCreditEarningsBase === 'cash' ? 'cash wages' : 'gross employee compensation'} − eligible adults × {dollars(settings.adultCreditPhaseOutStartPerAdult)}) × {percent(settings.adultCreditPhaseOutRate)}<br />statutory credit = {settings.adultCreditMode === 'universal' ? 'maximum' : 'max(0, phase-in − phaseout)'}<br /><br />Weighted statutory credits = {billions(result.adultCreditStatutoryCost)}<br />× {percent(settings.adultCreditTakeUpRate)} take-up<br />= {billions(result.adultCreditCost)} budget cost.</Formula></Audit>
  </section>;
}
